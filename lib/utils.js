...
export function createJWT(user) {
  ...
  const signedToken = jsonwebtoken.sign(payload, process.env.SECRET_JWT, {
    expiresIn: expiresIn,
    algorithm: "HS256",
  });
...
export function deleteLocalUploads(path) {
  fs.unlink(path, (err) => {
    if (err) {
      console.error(`Error deleting file ${path} on Uploads`);
    }
  });
}
